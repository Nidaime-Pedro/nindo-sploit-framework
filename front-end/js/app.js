...
const app = Vue.createApp({
    ...
    methods: {
        ...
        getPortDetails(port, property) {
            if (!this.portScanner.results || !this.portScanner.results.port_details) return 'N/A';

            const details = this.portScanner.results.port_details[port] || {};
            return details[property] || 'N/A';
        },
        ...
        // Obter classe CSS com base no status HTTP
        ...
        // Salvar resultados do scan atual
        saveResults() {
            ...
            switch (this.currentTool) {
                ...
                case 'directory-scanner':
                    ...
                    break;
                // Adicionar outros scanners aqui conforme implementados
            }

            if (currentResults) {
                this.savedResults.push(JSON.parse(JSON.stringify(currentResults)));
                this.saveSavedResultsToStorage();
                alert('Resultados salvos com sucesso!');
            }
        },

        // Limpar resultados do scan atual
        clearResults() {
            switch (this.currentTool) {
                case 'port-scanner':
        ...
        downloadReport() {
            ...
            if (this.reportGenerator.format === 'markdown') {
                ...
                link.href = url;
                link.download = `nsf_report_${new Date().toISOString().slice(0, 10)}.md`;
                link.click();
                ...
                window.open(`${PHP_API_URL}/download_report.php?file=${encodeURIComponent(filePath)}`, '_blank');
            }
        }
    }
});
